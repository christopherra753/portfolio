  ...
  return (
    <nav className='h-20 p-5 fixed w-full top-0 z-10 bg-white/80 dark:bg-neutral-800/80 backdrop-blur-md'>
      <div className='max-w-5xl h-full mx-auto flex  items-center justify-between'>
        ...
        <ul className={`z-10 fixed top-0 right-0 bg-gray-100 dark:bg-neutral-900 p-5 md:p-0 w-1/2 h-screen flex flex-col items-center justify-center gap-5 transition-all ${menu ? 'translate-x-0' : 'translate-x-full'} md:static md:h-auto md:w-auto md:bg-transparent md:flex-row md:translate-x-0`}>
        ...
        <div className='flex items-center gap-2'>
          <a
            href='/curriculum.pdf'
            download='curriculum'
            className='bg-orange-500 hover:bg-orange-600 transition-colors text-sm sm:text-base text-white px-4 py-1 rounded'
          >Descargar CV
          </a>
          <button onClick={handleTheme} className='group relative rounded-lg p-1 hover:bg-gray-100 dark:hover:bg-neutral-900/40 transition-colors'>
            <RiMoonLine className='block dark:hidden text-3xl' />
            <RiSunLine className='hidden dark:block text-3xl text-white' />
            <p className='absolute opacity-0 group-hover:opacity-100 -bottom-4 left-1/2 -translate-x-1/2 text-xs min-w-max pointer-events-none font-semibold transition-all'>
          ...
          <button onClick={() => setMenu(!menu)} className='block md:hidden z-10'>
            ...
            <RiCloseLine className={`${menu ? 'block' : 'hidden'} text-3xl dark:text-white hover:text-red-500 dark:hover:text-red-500 transition-colors`} />
          </button>
        </div>
      </div>
    </nav>
